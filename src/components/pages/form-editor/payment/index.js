import React, { useState } from "react";
import { Button } from "antd";
import ViewAll from "../view-all";
import NumericInput from "../../../common/numericInput";

const Payment = () => {
  const [value, setValue] = useState();

  const onChange = (value) => {
    setValue(value);
  };

  return (
    <div className="row">
      <div className="col-md-6">
        <ViewAll />
      </div>
      <div className="col-md-6 overflowY-scroll bg-white">
        <div className="edit-view-block p-4">
          <div className="d-flex justify-content-end">
            <div className="btn-shadow d-flex align-items-center no-wrap mr-2 px-2">
              <div className="logic-icon icon icon-m"></div>Test logic
            </div>
            <div className="btn-shadow icon reload-icon d-flex icon-l"></div>
          </div>
          <div className="my-5">
            <h5 className="fw-700">14. Can we know your card information?</h5>
            <p className="text-muted fw-700 m-0">
              Your credit card will be charged:
              <span className="text-dark"> $0.00</span>
            </p>
            <p className="text-muted fw-700 mb-4">
              We never store your card number or CVC number
            </p>
            <p className="text-orange d-flex align-items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="21"
                viewBox="0 0 16 21"
                fill="none"
                className="mr-2"
              >
                <path
                  d="M14 7H13V5C13 2.24 10.76 0 8 0C5.24 0 3 2.24 3 5V7H2C0.9 7 0 7.9 0 9V19C0 20.1 0.9 21 2 21H14C15.1 21 16 20.1 16 19V9C16 7.9 15.1 7 14 7ZM8 16C6.9 16 6 15.1 6 14C6 12.9 6.9 12 8 12C9.1 12 10 12.9 10 14C10 15.1 9.1 16 8 16ZM5 7V5C5 3.34 6.34 2 8 2C9.66 2 11 3.34 11 5V7H5Z"
                  fill="#F15A29"
                />
              </svg>
              Secured by Stripe
            </p>
          </div>
          <div className="payment-card-ui p-3 mb-5">
            <div className="mb-5">
              <NumericInput
                value={value}
                onChange={onChange}
                placeholder="Name on the card"
              />
            </div>
            <div className="mb-5">
              <NumericInput
                value={value}
                onChange={onChange}
                placeholder="Card number"
              />
            </div>

            <div className="row">
              <div class="col-md-6">
                <NumericInput
                  value={value}
                  onChange={onChange}
                  placeholder="Expiry date"
                />
              </div>
              <div class="col-md-6">
                <NumericInput
                  value={value}
                  onChange={onChange}
                  placeholder="Security code"
                />
              </div>
            </div>
          </div>
          <div className="d-block">
            <Button className="primary-org-button btn-m d-flex align-items-center">
              Submit
            </Button>
          </div>
        </div>
      </div>
    </div>
  );
};
export default Payment;
